...
function make_distance_matrix(datasets, fit_results, v_ranges, θh_ranges, P_ranges, neuron_categorization; s_weight=2, v_weight=1, min_P_rng=[0,2], use_all_ranges=false, rngs_valid=nothing)
    ...
    for idx1=1:size(distance_matrix,1)-1
        for idx2=idx1+1:size(distance_matrix,1)
            ...
            distance_matrix[idx1,idx2] = v_weight * sum([sum(n_cat["v_encoding"][i,i+1,:,:]) for i=1:3]) * σ_vel / (v_range[4] - v_range[1])
            distance_matrix[idx1,idx2] += 2 * sum(n_cat["rev_θh_encoding_act"]) * σ_θh / (θh_range[2] - θh_range[1])
            distance_matrix[idx1,idx2] += 2 * sum(n_cat["rev_θh_encoding_inh"]) * σ_θh / (θh_range[2] - θh_range[1])
            distance_matrix[idx1,idx2] += 2 * sum(n_cat["fwd_θh_encoding_act"]) * σ_θh / (θh_range[2] - θh_range[1])
            distance_matrix[idx1,idx2] += 2 * sum(n_cat["fwd_θh_encoding_inh"]) * σ_θh / (θh_range[2] - θh_range[1])
            distance_matrix[idx1,idx2] += 2 * sum(n_cat["rev_P_encoding_act"]) * σ_pumping / (P_range[2] - P_range[1])
            distance_matrix[idx1,idx2] += 2 * sum(n_cat["rev_P_encoding_inh"]) * σ_pumping / (P_range[2] - P_range[1])
            distance_matrix[idx1,idx2] += 2 * sum(n_cat["fwd_P_encoding_act"]) * σ_pumping / (P_range[2] - P_range[1])
            distance_matrix[idx1,idx2] += 2 * sum(n_cat["fwd_P_encoding_inh"]) * σ_pumping / (P_range[2] - P_range[1])
